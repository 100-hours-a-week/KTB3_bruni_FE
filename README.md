<div align= "center">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=15bee0&height=120&text=Bruni's%20Board&animation=&fontColor=000000&fontSize=70" />
    </div>
    <div align= "center"> 
    <h2 style="border-bottom: 1px solid #d8dee4; color: #282d33;">  </h2>  
    <div style="font-weight: 700; font-size: 15px; text-align: center; color: #282d33;">  </div> 
    </div>
    <div align= "center">
    <h2 style="border-bottom: 1px solid #d8dee4; color: #282d33;"> 🛠️ Tech Stacks </h2> <br> 
    <div style="margin: 0 auto; text-align: center;" align= "center"> <img src="https://img.shields.io/badge/Javascript-F7DF1E?style=for-the-badge&logo=Javascript&logoColor=white">
          <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=HTML5&logoColor=white">
          <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=CSS3&logoColor=white">
          </div>
    </div>
    
<div align= "center"> 
    <h2 style="border-bottom: 1px solid #d8dee4; color: #282d33;"> Bruni's Board </h2>   
    <div style="font-weight: 700; font-size: 18px; text-align: center; color: #282d33;"> 누구나 글을 작성할 수 있고, 공유할 수 있는 오픈 커뮤니티 </div> 
    </div> 
    
<br>
    
## 🌈프로젝트 소개
Bruni's Board 는 오픈 커뮤니티 웹 애플리케이션입니다. 누구나 글과 사진을 통해 공유하고 이야기를 나눌 수 있는 서비스입니다.   

<br>   

## 📌주요 기능   

| 기능 | 설명 |
| --- | --- | 
| **사용자** | •회원가입/로그인/로그아웃<br>•회원정보 수정/탈퇴<br>•인증(쿠키-세션) | 
| **게시판** | •게시글 CRUD (이미지 업로드)<br>•조회수/좋아요/댓글수 표시| 
| **게시글** | •좋아요, 댓글(작성/수정/삭제) |
 
<br>
<div style="text-align: left;"> 
  <h2 style="border-bottom: 1px solid #d8dee4; color: #282d33;"> 🎥프로젝트 시연 영상 </h2>  

  <div align="left">
    
  [![Bruni's Board 데모 영상](https://img.youtube.com/vi/T9VYQ6cBi64/0.jpg)](https://www.youtube.com/watch?v=T9VYQ6cBi64)
  
  </div>
</div>  



   
    
